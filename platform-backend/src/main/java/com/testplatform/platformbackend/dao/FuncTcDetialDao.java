...
@Repository
public class FuncTcDetialDao {
    @Autowired
    private final JdbcTemplate jdbcTemplate;

    public FuncTcDetialDao(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    public List<FuncTcDetial> getDetial(int id){
        String sql = "select * from function_tc_detial where id = "+id;
    ...
    public String getPre(int id){
        String sql = "select * from function_tc_pre where id ="+id;
//        System.out.println(((FuncTcDetial)jdbcTemplate.query(sql, new BeanPropertyRowMapper(FuncTcDetial.class)).get(0)).getPre());
    ...
    public int addRow(int id){
        String sql = "insert into function_tc_detial(id,steps,expected,actual) values(?,'无','无','无');";
        return jdbcTemplate.update(sql,id);
    }

    public int deleteRow(int xid){
        String sql = "delete from function_tc_detial where xid = ?";
        return jdbcTemplate.update(sql,xid);
    }

    public void updateDetial(List<FuncTcDetial> detials){
        String sql = "update function_tc_detial set steps = ?,expected = ?,actual = ? where id = ? and xid = ?";
